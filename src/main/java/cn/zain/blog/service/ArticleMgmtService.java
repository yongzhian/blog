...
import cn.zain.blog.model.ArchiveDate;
import cn.zain.blog.model.Common;
import cn.zain.blog.model.Tag;
import cn.zain.blog.model.UserExt;
import cn.zain.blog.repository.ArchiveDateRepository;
import cn.zain.blog.repository.CommentRepository;
import cn.zain.blog.repository.TagArticleRepository;
import cn.zain.blog.util.Comments;
...
import cn.zain.blog.event.EventTypes;
import cn.zain.blog.model.Article;
import cn.zain.blog.model.Comment;
import cn.zain.blog.model.Option;
import cn.zain.blog.repository.ArchiveDateArticleRepository;
import cn.zain.blog.repository.ArticleRepository;
import cn.zain.blog.repository.TagRepository;
...
public class ArticleMgmtService {
    ...
    public void cancelPublishArticle(final String articleId) throws ServiceException {
        ...
        try {
            ...
            final int articleCmtCnt = article.getInt(Article.ARTICLE_COMMENT_COUNT);
    ...
    public void topArticle(final String articleId, final boolean top) throws ServiceException {
        ...
        try {
            ...
            topArticle.put(Article.ARTICLE_PUT_TOP, top);
    ...
    public void updateArticle(final JSONObject requestJSONObject) throws ServiceException {
        ...
        try {
            ...
            final String permalink = getPermalinkForUpdateArticle(oldArticle, article, (Date) oldArticle.get(Article.ARTICLE_CREATE_DATE));
            ...
            // Set date
            article.put(Article.ARTICLE_UPDATE_DATE, oldArticle.get(Article.ARTICLE_UPDATE_DATE));
            ...
            if (article.getBoolean(Article.ARTICLE_IS_PUBLISHED)) { // Publish it
            ...
            final boolean publishNewArticle = !oldArticle.getBoolean(Article.ARTICLE_IS_PUBLISHED) && article.getBoolean(Article.ARTICLE_IS_PUBLISHED);
     ...
     * Article#ARTICLE_RANDOM_DOUBLE}) of the specified
    ...
    private void fillAutoProperties(final JSONObject oldArticle, final JSONObject article) throws JSONException {
        final Date createDate = (Date) oldArticle.get(Article.ARTICLE_CREATE_DATE);

        article.put(Article.ARTICLE_CREATE_DATE, createDate);
        article.put(Article.ARTICLE_COMMENT_COUNT, oldArticle.getInt(Article.ARTICLE_COMMENT_COUNT));
        article.put(Article.ARTICLE_VIEW_COUNT, oldArticle.getInt(Article.ARTICLE_VIEW_COUNT));
        article.put(Article.ARTICLE_PUT_TOP, oldArticle.getBoolean(Article.ARTICLE_PUT_TOP));
        article.put(Article.ARTICLE_HAD_BEEN_PUBLISHED, oldArticle.getBoolean(Article.ARTICLE_HAD_BEEN_PUBLISHED));
        article.put(Article.ARTICLE_AUTHOR_EMAIL, oldArticle.getString(Article.ARTICLE_AUTHOR_EMAIL));
        article.put(Article.ARTICLE_RANDOM_DOUBLE, Math.random());
    }
    ...
    private String getPermalinkForUpdateArticle(final JSONObject oldArticle, final JSONObject article, final Date createDate)
        ...
        final String articleId = article.getString(Keys.OBJECT_ID);
        String ret = article.optString(Article.ARTICLE_PERMALINK).trim();
        final String oldPermalink = oldArticle.getString(Article.ARTICLE_PERMALINK);
