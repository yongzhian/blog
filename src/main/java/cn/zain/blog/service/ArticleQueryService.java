...
import cn.zain.blog.model.Sign;
...
import cn.zain.blog.util.comparator.Comparators;
...
public class ArticleQueryService {
    ...
    public JSONObject getArticles(final JSONObject requestJSONObject) throws ServiceException {
        ...
        try {
            ...
            final boolean articleIsPublished = requestJSONObject.optBoolean(Article.ARTICLE_IS_PUBLISHED, true);

            final Query query = new Query().setCurrentPageNum(currentPageNum).setPageSize(pageSize).addSort(Article.ARTICLE_PUT_TOP, SortDirection.DESCENDING).addSort(Article.ARTICLE_CREATE_DATE, SortDirection.DESCENDING).setFilter(
                    new PropertyFilter(Article.ARTICLE_IS_PUBLISHED, FilterOperator.EQUAL, articleIsPublished));
            ...
            for (int i = 0; i < articles.length(); i++) {
                ...
                article.put(Article.ARTICLE_CREATE_TIME, ((Date) article.get(Article.ARTICLE_CREATE_DATE)).getTime());
                article.put(Article.ARTICLE_UPDATE_TIME, ((Date) article.get(Article.ARTICLE_UPDATE_DATE)).getTime());
    ...
    public String getArticleContent(final HttpServletRequest request, final String articleId) throws ServiceException {
        ...
        try {
            ...
            } else if ("CodeMirror-Markdown".equals(article.optString(Article.ARTICLE_EDITOR_TYPE))) {
                // Markdown to HTML for content and abstract
                Stopwatchs.start("Get Article Content [Markdown]");
                final String content = article.optString(Article.ARTICLE_CONTENT);

                article.put(Article.ARTICLE_CONTENT, Markdowns.toHTML(content));
                Stopwatchs.end();
            }
    ...
    public void markdown(final JSONObject article) throws Exception {
        if ("CodeMirror-Markdown".equals(article.optString(Article.ARTICLE_EDITOR_TYPE))) {
            ...
            final String abstractContent = article.optString(Article.ARTICLE_ABSTRACT);

            if (!Strings.isEmptyOrNull(abstractContent)) {
                Stopwatchs.start("Abstract");
                article.put(Article.ARTICLE_ABSTRACT, Markdowns.toHTML(abstractContent));
                Stopwatchs.end();
            }
